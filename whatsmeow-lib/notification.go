...
// resolveTCTokenStorageLID resolves a phone-number JID to its LID so privacy
// tokens are always stored/looked up under a consistent key. WhatsApp delivers
// privacy tokens addressed by LID (sender_lid), while the send path addresses
// recipients by phone number; without this resolution the stored token is never
// found on the next outbound send, causing error 463 (NackCallerReachoutTimelocked).
// If the JID isn't a phone number or no LID mapping is known, the (non-AD) input is returned.
...
func (cli *Client) handlePrivacyTokenNotification(ctx context.Context, node *waBinary.Node) {
	...
	for _, child := range tokens.GetChildren() {
		...
		if err != nil {
			...
			cli.Log.Debugf("Stored privacy token from %s (ts: %v)", senderLID, timestamp)
		}
	}
}
