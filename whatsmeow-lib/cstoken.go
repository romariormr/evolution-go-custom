...
// generateCsToken derives a <cstoken> for the given JID using HMAC-SHA256(nctSalt, recipientLID).
// This is the fallback trust token attached to outgoing DMs to cold contacts (no stored tctoken),
// required to avoid WhatsApp error 463 (NackCallerReachoutTimelocked).
func (cli *Client) generateCsToken(ctx context.Context, jid types.JID) []byte {
	if !shouldSendCsToken(jid) {
		return nil
	}
	if cli.Store == nil || cli.Store.NCTSalt == nil || cli.Store.LIDs == nil {
		return nil
	}
...
func (cli *Client) storeNCTSalt(ctx context.Context, salt []byte) error {
	...
	cli.Log.Infof("Stored new NCT salt (%d bytes) for cstoken derivation", len(salt))
	return nil
}
