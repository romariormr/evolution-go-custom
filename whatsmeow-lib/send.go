...
func (cli *Client) sendDM(
	...
	// Resolver PN->LID para buscar o token sob a mesma chave em que foi gravado
	// (ver resolveTCTokenStorageLID); sem isso o token recebido nunca é encontrado
	// e o envio para contato "frio" falha com erro 463 (NackCallerReachoutTimelocked).
	var tcToken *store.PrivacyToken
	if cli.Store != nil && cli.Store.PrivacyTokens != nil {
		storageJID := cli.resolveTCTokenStorageLID(ctx, to)
		var err error
		tcToken, err = cli.Store.PrivacyTokens.GetPrivacyToken(ctx, storageJID)
		if err != nil {
	...
	if tcToken != nil {
		...
		// Cold contact: no stored tctoken, fall back to a locally-derived <cstoken>
		// so WhatsApp doesn't reject the send with error 463 (NackCallerReachoutTimelocked).
		cli.Log.Infof("Attaching cstoken for cold contact %s", to)
		node.Content = append(node.GetChildren(), waBinary.Node{
			Tag:     "cstoken",
			Content: csToken,
		})
