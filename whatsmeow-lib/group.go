...
func (cli *Client) CreateGroup(ctx context.Context, req ReqCreateGroup) (*types.GroupInfo, error) {
	...
	for i, participant := range req.Participants {
		...
		if cli.Store != nil && cli.Store.PrivacyTokens != nil {
			pt, err = cli.Store.PrivacyTokens.GetPrivacyToken(ctx, cli.resolveTCTokenStorageLID(ctx, participant))
			if err != nil {
...
func (cli *Client) parseGroupNode(groupNode *waBinary.Node) (*types.GroupInfo, error) {
	...
	// WhatsApp occasionally omits `creation` for legacy/community groups in
	// the participating-groups response. Treat it as unknown instead of
	// turning an otherwise valid group into a parse warning.
